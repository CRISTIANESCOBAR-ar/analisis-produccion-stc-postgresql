...
-- ========== USTER_PAR ==========
CREATE TABLE IF NOT EXISTS uster_par (
...
CREATE INDEX IF NOT EXISTS idx_uster_par_lote ON uster_par(lote);
CREATE INDEX IF NOT EXISTS idx_uster_par_timestamp ON uster_par(time_stamp);

CREATE TRIGGER uster_par_updated_at BEFORE UPDATE ON uster_par
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- ========== USTER_TBL ==========
CREATE TABLE IF NOT EXISTS uster_tbl (
    id SERIAL PRIMARY KEY,
    testnr VARCHAR(20) REFERENCES uster_par(testnr) ON DELETE CASCADE,
    seqno INTEGER,
    no_ INTEGER,
...
CREATE INDEX IF NOT EXISTS idx_uster_tbl_testnr ON uster_tbl(testnr);
CREATE INDEX IF NOT EXISTS idx_uster_tbl_seqno ON uster_tbl(testnr, seqno);

CREATE TRIGGER uster_tbl_updated_at BEFORE UPDATE ON uster_tbl
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- ========== TENSORAPID_PAR ==========
CREATE TABLE IF NOT EXISTS tensorapid_par (
    testnr VARCHAR(20) PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_tensorapid_par_lote ON tensorapid_par(lote);
CREATE INDEX IF NOT EXISTS idx_tensorapid_par_timestamp ON tensorapid_par(time_stamp);

CREATE TRIGGER tensorapid_par_updated_at BEFORE UPDATE ON tensorapid_par
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- ========== TENSORAPID_TBL ==========
CREATE TABLE IF NOT EXISTS tensorapid_tbl (
    id SERIAL PRIMARY KEY,
    testnr VARCHAR(20) REFERENCES tensorapid_par(testnr) ON DELETE CASCADE,
    huso_number INTEGER,
...
CREATE INDEX IF NOT EXISTS idx_tensorapid_tbl_testnr ON tensorapid_tbl(testnr);
CREATE INDEX IF NOT EXISTS idx_tensorapid_tbl_huso ON tensorapid_tbl(testnr, huso_number);

CREATE TRIGGER tensorapid_tbl_updated_at BEFORE UPDATE ON tensorapid_tbl
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
COMMENT ON TABLE uster_par IS 'Parámetros ensayos Uster (Oracle)';
COMMENT ON TABLE uster_tbl IS 'Datos detallados Uster: U%, CVm%, irregularidades';
COMMENT ON TABLE tensorapid_par IS 'Parámetros ensayos TensoRapid (Oracle)';
COMMENT ON TABLE tensorapid_tbl IS 'Datos TensoRapid: fuerza, elongación, tenacidad';
